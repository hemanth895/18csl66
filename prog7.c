...
if(st[i]>tq) // when service time of a process greater 
than time //quantum then time
...
for(i=0;i<n;i++) // to calculate the wait time and turnaround time of eachprocess
{
wt[i]=tat[i]-bt[i]; // waiting time calculated from the turnaround time -bursttime
...
for(j = i + 1; j <= n; j++)
{
if(process[i].arrival > process[j].arrival) // sort arrival time of process
{
temp = process[i];
process[i] = process[j];
process[j] = temp;
